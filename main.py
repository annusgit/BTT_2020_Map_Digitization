...
def get_digitized_clipped_map(this_path, this_shape):
    ...
    ds = gdal.Open(this_path)
    rgb = [ds.GetRasterBand(x).ReadAsArray() for x in range(1, 4)]
    clipped_map = np.dstack(rgb)
    ...
    return [ds, clipped_map, forest_map, forest_percent]
...
if __name__ == "__main__":
    ...
    path_1, path_2 = "E:\Forest Cover - Redo 2020", "Clipped_images_and_maps"
    ...
    for district in all_districts:
        ...
        original_ds, clipped_map, forest_map, forest_percentage = get_digitized_clipped_map(this_path=os.path.join(path_1, path_2, f"{district}_map.tif"),
                                                                                            this_shape=(stretched_image.shape[1], stretched_image.shape[0]))
        ...
        print(f"{district} -> Shapes: ({stretched_image.shape[0]}, {stretched_image.shape[1]}) & ({forest_map.shape[0]}, {forest_map.shape[1]})")
        print("----------------------------------------------------------------------------------------------")
        ...
        axarr[1].imshow(clipped_map)
        axarr[2].imshow(forest_map)
        plt.title("Forest Percentage: {:.2f}%".format(forest_percentage))
        plt.show()
        ...
        # matimg.imsave(f'Forest_Maps/{district}_2015.png', forest_map_for_visualization)
        # plt.imshow(forest_map_for_visualization)
        # plt.show()
        # print(f'Saved Forest_Maps/{district}_2015.png')
        pass
    # data to plot
